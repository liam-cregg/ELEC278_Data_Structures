...
// Given data in an array, turn it into a heap
{
	...
	h->maxSize = size;
	h->a = a;
	int start = (size - 1) / 2; //get greatest non-leaf node

	for(i = start; i >= 0; i--){
		reheapDown(h, i);
	}

	printf("Array heaped to");
	print(h);

	/*for(i=0;i<size;i++){ 		reheap up method - change h->last to 0 if using this
		reheapUp(h,h->last);
		//print(h);
		h->last++;
	}
...
int main()
{
    ...
    while(findMin(h) < k)
    {
        ...
        {
            int cookie1 = withdrawMin(h);
            int cookie2 = withdrawMin(h);
            insert(h, (1*cookie1 + 2*cookie2));
            ctr++;
        }
    }
